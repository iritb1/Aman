...
export class JokeTableComponent implements OnInit {
  ...
  openDialog(joke: Joke , threeRandomJokes: string[] | TwoPart[]){
    this.dialog.open(PopupComponent, {
      width: '700px',
      height: '500px',
      data: {joke , threeRandomJokes},
    });
  }

  onClickJoke(joke: Joke): void {
    const threeRandomJokes: any[] = [];
    switch(joke.type) {
      case JokeType.Signle:
              for(let i = 0 ; i < 3 ; i ++){
                const index = Math.floor(Math.random() * (this.single.length)) + 1;
                threeRandomJokes.push(this.single[index]);
              }
              this.openDialog(joke,threeRandomJokes); 
              break;

      case JokeType.Twopart:
              for(let i = 0 ; i < 3 ; i ++){
                const index = Math.floor(Math.random() * (this.twopart.length)) + 1;
                threeRandomJokes.push(this.twopart[index]);
              }
              this.openDialog(joke,threeRandomJokes);
              break;
    }
